...
%% === SPECTRAL ANALYSIS WITH COLORED VLF, LF, HF BANDS ===
if exist('RR_resampled','var') && ~isempty(RR_resampled)
    ...
    % Compute PSD (Welch)
    fs_resample = 4; 
    window = hamming(round(length(RR_detrended)/8));
    noverlap = round(length(window)/2);
    nfft = max(256, 2^nextpow2(length(RR_detrended)));

    [Pxx, f] = pwelch(RR_detrended, window, noverlap, nfft, fs_resample);
    ...
    % === BEAUTIFUL PLOT WITH COLORED AREAS ===
    figure('Position',[100 100 950 450]);

    % Plot PSD curve
    plot(f, Pxx, 'k', 'LineWidth', 2); hold on;

    % Color VLF region (Orange)
    area(f(f>=VLF_band(1) & f<=VLF_band(2)), ...
         Pxx(f>=VLF_band(1) & f<=VLF_band(2)), ...
         'FaceColor',[1 0.6 0.1], 'FaceAlpha',0.4, 'EdgeColor','none');

    % Color LF region (Red)
    area(f(f>=LF_band(1) & f<=LF_band(2)), ...
         Pxx(f>=LF_band(1) & f<=LF_band(2)), ...
         'FaceColor',[1 0.2 0.2], 'FaceAlpha',0.4, 'EdgeColor','none');

    % Color HF region (Blue)
    area(f(f>=HF_band(1) & f<=HF_band(2)), ...
         Pxx(f>=HF_band(1) & f<=HF_band(2)), ...
         'FaceColor',[0.2 0.4 1], 'FaceAlpha',0.4, 'EdgeColor','none');

    xlabel('Frequency (Hz)');
    ylabel('Power Spectral Density (s^2/Hz)');
    title('Power Spectral Density of RR Intervals (VLF, LF, HF Bands Highlighted)');
    
    legend('PSD','VLF (0.0033–0.04 Hz)','LF (0.04–0.15 Hz)','HF (0.15–0.40 Hz)', ...
           'Location','northeast');
    grid on; xlim([0 0.5]);
